...
\begin{tikzpicture}[
    ...
    logitsBox/.style={draw, thick, fill=lightgray, text width=5cm, minimum height=1cm, align=center, rounded corners},
    noteBox/.style={draw, thick, fill=lightgray!50, text width=5cm, minimum height=1cm, align=center, rounded corners},
    ]
    
    % Title
    \node[font=\large\bfseries] (title) at (9,6) {Whisper Yes/No Strategy Classification Models};
    ...
    \node[promptBox, fill=promptgreen, below=0.5cm of audio] (prompt_prefix) {
    ...
    % Last Token Logits (final output)
    \node[logitsBox, right=1.5cm of whisper] (classify) {Last Token's Normalized\\Yes/No Logits};
    ...
    \draw[arrow] (whisper) -- (classify);
    
    % Model Variants - ST above MTBL
    \node[noteBox, below=1.0cm of whisper] (st_note) {
