...
args.temp_dir = f"/dev/shm/bert/tmp"
...
def train(
    ...
    if worker_id == 0:
        wandb.init(project="werewolf", name=f"bert-{args.dataset}-seed{args.seed}-{strategy}", tags=["bert", args.dataset, f"seed{args.seed}", strategy], config=vars(args))
