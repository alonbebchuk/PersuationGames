...
args.out_dir = os.path.join(ROOT_DIR, f"out/bert/multi_task_multi_label/{args.seed}")
...
def train(tokenizer: BertTokenizer, model: FlaxBertForSequenceClassification) -> None:
    ...
    if worker_id == 0:
        wandb.init(project="werewolf", name=f"bert-mtml-seed{args.seed}", tags=["bert", "mtml", f"seed{args.seed}"], config=vars(args))
...
def main() -> None:
    try:
        ...
        model = FlaxBertForSequenceClassification.from_pretrained(model_name, problem_type="multi_label_classification", num_labels=len(STRATEGIES))
