...
class PromptBuilder:
    def __init__(self, args: Any, tokenizer: BertTokenizer):
        self.args = args
        self.tokenizer = tokenizer

        self.prompt_prefix_tokens_dict = {"No Strategy": [tokenizer.cls_token]}
        self.prompt_suffix_tokens_dict = {"No Strategy": [tokenizer.sep_token]}
        for strategy, strategy_definition in STRATEGY_TO_STRATEGY_DEFINITION.items():
            ...
            self.prompt_prefix_tokens_dict[strategy] = [tokenizer.cls_token] + tokenizer.tokenize(prompt_prefix)
            self.prompt_suffix_tokens_dict[strategy] = tokenizer.tokenize(prompt_suffix) + [tokenizer.sep_token]
