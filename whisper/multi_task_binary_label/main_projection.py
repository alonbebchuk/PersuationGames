...
args.out_dir = os.path.join(ROOT_DIR, f"out/whisper/projection/multi_task_binary_label/{args.seed}")
...
def train(tokenizer: WhisperTokenizer, feature_extractor: WhisperFeatureExtractor, model: FlaxWhisperForSequenceClassification) -> None:
    ...
    if worker_id == 0:
        wandb.init(project="werewolf", name=f"whisper-projection-mtbl-seed{args.seed}", tags=["whisper", "projection", "mtbl", f"seed{args.seed}"], config=vars(args))
