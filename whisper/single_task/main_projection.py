...
args.out_dir = os.path.join(ROOT_DIR, f"out/whisper/projection/single_task/{args.strategy}/{args.seed}")
...
def train(tokenizer: WhisperTokenizer, feature_extractor: WhisperFeatureExtractor, model: FlaxWhisperForSequenceClassification) -> None:
    ...
    if worker_id == 0:
        wandb.init(project="werewolf", name=f"whisper-projection-{args.strategy}-seed{args.seed}", tags=["whisper", "projection", args.strategy, f"seed{args.seed}"], config=vars(args))
