...
args.out_dir = os.path.join(ROOT_DIR, f"out/whisper/single_task/v2/{args.strategy}/{args.seed}")
...
def train(tokenizer: WhisperTokenizer, feature_extractor: WhisperFeatureExtractor, model: FlaxWhisperForSequenceClassification) -> None:
    ...
    if worker_id == 0:
        wandb.init(project="werewolf-single-task", name=f"whisper-v2-{args.strategy}-seed{args.seed}", tags=["whisper", "v2", args.strategy, f"seed{args.seed}"], config=vars(args))
