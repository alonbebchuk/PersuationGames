...
args.out_dir = os.path.join(ROOT_DIR, f"out/whisper/yes_no/single_task/{args.strategy}/{args.seed}")
...
def train(tokenizer: WhisperTokenizer, feature_extractor: WhisperFeatureExtractor, model: FlaxWhisperForConditionalGeneration) -> None:
    ...
    if worker_id == 0:
        wandb.init(project="werewolf", name=f"whisper-yes_no-{args.strategy}-seed{args.seed}", tags=["whisper", "yes_no", args.strategy, f"seed{args.seed}"], config=vars(args))
